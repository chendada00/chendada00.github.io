(function () {
    ...
    async function readAloud() {
        ...
        try {
            ...
            if (response.status === 403) {
                console.error("403 refer与key不匹配11。");
    ...
    async function aiAbstractTianli(num) {
        ...
        try {
            ...
            // Animation to show "Generating..." while waiting for response
            const animationTextElement = document.createElement('div');
            animationTextElement.id = 'ai-animation-text';
            explanation.innerHTML = "";  // Clear any previous content
            explanation.appendChild(animationTextElement);

            animationInterval = setInterval(() => {
                animationTextElement.innerHTML = "生成中" + ".".repeat(indexJ);
                indexJ = (indexJ % 3) + 1;
            }, 500);


            fetch('https://func-hhdfggh-xyuoaxukun.cn-hangzhou.fcapp.run', {
                // fetch('https://qianfan.vercel.app/', {
                method: 'post',
    ...
    function type() {
        ...
        if (char_index < text.length) {
            let txt = explanation.innerHTML;
            let cursor = enableCursor ? "|" : "";
            if (enableCursor && txt.endsWith("|")) {
                txt = txt.slice(0, -1);
            }
            explanation.innerHTML = txt + text.charAt(char_index) + cursor;
            char_index++;
    ...
    function onAiTagClick() {
        if (mode === "tianli") {
            ...
            startAI(
                `你好，我是本站摘要生成助理${gptName}，是一个基于GPT-4的生成式AI。我在这里只负责摘要的预生成和显示，你无法与我直接沟通。`
            );
        }
    }
